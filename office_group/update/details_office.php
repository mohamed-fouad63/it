...
$query_post_group = mysqli_query($conn, "SELECT post_group FROM all1 GROUP BY post_group HAVING COUNT(*) > 1");
...
?>
<form method="post" action="update_office_details.php">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select class="custom-select" name="post_group" >
          ...
          <?php  while($row_query_post_group=mysqli_fetch_assoc($query_post_group)){ ?>
            <option  value="<?php echo $row_query_post_group["post_group"];?>"
                    <?php if ($row_query_post_group["post_group"] == $post_group ){ echo 'selected="selected"';} ?>
                    
                ><?php echo $row_query_post_group["post_group"];?>
            </option>
           <?php } ?>
            </select>
        </div>
        ...
        <div class="input-group mb-4">
          ...
          <?php  while($row_query_id_hg=mysqli_fetch_assoc($query_id_hg)){ $hg_id_first_name = $row_query_id_hg["first_name"]; ?>
            <option  value="<?php echo $row_query_id_hg["id"];?>"
                  <?php if ( $hg_id_first_name == $hg_first_name ){ echo 'selected="selected"';} ?>
        ...
        <div class="input-group mb-4">
          ...
          <?php  while($row_query_id_it=mysqli_fetch_assoc($query_id_it)){ $it_id_first_name = $row_query_id_it["first_name"]; ?>
            <option  value="<?php echo $row_query_id_it["id"];?>"
                    <?php if ($it_id_first_name == $it_first_name){echo 'selected="selected"';} ?>
...
<div class="row">
    <div class="col-md-6 mb-3">
         <div class="input-group mb-3">
          <div class="input-group-prepend">
            <span class="input-group-text" id="basic-addon1">ايام العمل</span>
          </div> 
             <input type="text" name="sat" class="form-control details-text" value="<?php echo $Sat;?>">
        </div>
    </div>
    <div class="col-md-6 mb-3">
         <div class="input-group mb-3">
          <div class="input-group-prepend">
            <span class="input-group-text" id="basic-addon1">العنوان</span>
             ...
             <input type="text" name="address" class="form-control details-text" value="<?php echo $address;?>">
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-md-6 mb-3">
         <div class="input-group mb-3">
          <div class="input-group-prepend">
            <span class="input-group-text" id="basic-addon1">روتر لاسلكى</span>
          </div> 
             <input type="text" name="3G" class="form-control details-text" value="<?php echo $g3;?>">
        </div>
    </div> 
    <div class="col-md-6 mb-3">
        <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span class="input-group-text" id="basic-addon1">روتر ارضى</span>
            </div>
            <input type="text" name="LL" class="form-control details-text" value="<?php echo $ll;?>">
        </div>
    </div>
</div>
<div class="row">
...
<input class="btn btn-primary" type="submit" value="تحديث البيانات">
</form>
